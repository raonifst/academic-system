...
export class CoursesDAG {
  ...
  groupBy(maxCreditsPerSemester = 28) {
    const self = this;
    const coursesComparator = function (a, b) {
      if (a.group.length > 0 && b.group.length > 0) {
        const cA = self._gMap.get(a.group[0]);
        const cB = self._gMap.get(b.group[0]);
        if (cA.alunos == 0)
          return -1;
        if (cB.alunos == 0)
          return 1;
    ...
    const to = [];
    JSON.parse(JSON.stringify(this.getTopologicalOrder())).forEach(e => {
      to.push({ group: e, color: GraphColors.WHITE });
    }); // Deep copy
    var index = 1;
    ...
    while (counter > 0) {
      ...
      to.sort(coursesComparator); // Ordena os

      for (var j = 0; j < to.length; j++)
        to[j].color = GraphColors.WHITE;

      availableGroups = to.length;

      while (availableGroups > 0 && to.length > 0) {
        for (var k = 0; k < to.length; k++) {
          const courseCode = to[k].group[0]; // Recebe a primeira disciplina do grupo k
          if (!courseCode) {
            to.splice(k--, 1);
            continue;
          }
          ...
          if (to[k].color == GraphColors.WHITE) {
            groups.get(index).push(this._gMap.get(to[k].group.shift()).nome);
            to[k].color = GraphColors.GRAY;
            availableGroups--;
            counter--;
          }
        }
