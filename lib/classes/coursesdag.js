...
export class CoursesDAG {
  ...
  groupBy(maxCreditsPerSemester = 28, option = 1) {
    ...
    // TODO implementar funções de callback para ordenação (option)
    /* Ordenações callback
    * 1 ord: Por aprovação, por semestre, por créditos
    * 2 ord: Por semestre, por aprovação, por créditos
    * 3 ord: Por créditos, por semestre, por aprovação
    * */
    ...
    while (counter > 0) {
      ...
      for (k = 0; k < to.length; k++) {
        ...
        result[index].push(this._gMap.get(to[k].shift()).nome); // Remove do grupo k e adiciona no resultado
        counter--;
      }
