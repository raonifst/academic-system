...
export class CoursesDAG {
  ...
  groupBy(maxCreditsPerSemester = 28) {
    ...
    const to = JSON.parse(JSON.stringify(this.getTopologicalOrder())); // Deep copy
    var index = 1;
    ...
    while (counter > 0) {
      ...
      for (var k = 0; k < to.length; k++) {
        ...
        groups.get(index).push(this._gMap.get(to[k].shift()).nome);
        counter--;
      }
    }
    console.log("Contador: " + tempCounter);
    return groups;
  }
