...
export class CoursesDAG {
  ...
  groupTopCourses(maxCreditsPerSemester = 28, option = 1) {
    const self = this;
    const result = [];
    const topOrd = deepCopy(this.getTopologicalOrder());
    ...
    if (this.size() > 0) {
      topOrd.sort(sortCoursesOptions[option - 1]); // Ordena os grupos de disciplina
      for (var k = 0; k < topOrd.length; k++) {
        const topCourse = topOrd[k][0]; // Recebe a primeira disciplina do grupo k
        creditsCounter += this._gMap.get(topCourse).creditos;
        if (creditsCounter > maxCreditsPerSemester) {
          creditsCounter = 0;
          break;
        }
        result.push(this._gMap.get(topOrd[k].shift())); // Remove do grupo k e adiciona no resultado
      }
    }
    return result;
  }
