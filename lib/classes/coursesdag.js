    ...
    for (var j = 1; j <= this.size(); j++) {
      for (var k = 0; k < topologicalOrder.length; k++) {
        ...
        if (courseCode) {
          ...
          groups.get(index).push(topologicalOrder[k].shift());
          // TODO a partir daqui, implementar algoritmo que quebra grupo baseado nos tempos
        }
      }
    }
    // TODO remover linhas de debug após conclusão de projeto do algoritmo
    console.log("Local:");
    console.log(topologicalOrder);
    console.log("Attr (topological order):");
    console.log(this.getTopologicalOrder());
    return groups;
  }
}
